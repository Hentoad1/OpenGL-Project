...
sVertex::sVertex(float x, float y, float z, float uvx, float uvy, float nx, float ny, float nz) :
	position_x(x),
	position_y(y),
	position_z(z),
...
sVertex::sVertex(const aiVector3D& pos, const aiVector3D& uv, const aiVector3D& normal) :
	position_x(pos.x),
	position_y(pos.y),
	position_z(pos.z),
...
void sVertex::SetPosition(const glm::vec3& pos) {
	position_x = pos.x;
	position_y = pos.y;
	position_z = pos.z;
}

void sVertex::SetPosition(const aiVector3D& pos) {
	position_x = pos.x;
	position_y = pos.y;
	position_z = pos.z;
}

void sVertex::SetTexture(const glm::vec2& uv) {
	texture_x = uv.x;
	texture_y = uv.y;
}

void sVertex::SetTexture(const aiVector3D& uv) {
	texture_x = uv.x;
	texture_y = uv.y;
}

void sVertex::SetNormal(const glm::vec3& normal) {
	normal_x = normal.x;
	normal_y = normal.y;
	normal_z = normal.z;
}

void sVertex::SetNormal(const aiVector3D& normal) {
	normal_x = normal.x;
	normal_y = normal.y;
	normal_z = normal.z;
}

glm::vec3 sVertex::GetPosition() const {
	return glm::vec3(position_x, position_y, position_z);
}

glm::vec2 sVertex::GetTexture() const {
	return glm::vec2(texture_x, texture_y);
}

glm::vec3 sVertex::GetNormal() const {
	return glm::vec3(normal_x, normal_y, normal_z);
}